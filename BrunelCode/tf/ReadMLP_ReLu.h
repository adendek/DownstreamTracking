...
class ReadMLP_ReLu : public IClassifierReader {

 public:

   // constructor
   ReadMLP_ReLu( std::vector<std::string>& theInputVars )
      : IClassifierReader(),
        fClassName( "ReadMLP_ReLu" ),
        fNvars( 11 ),
        fIsNormalised( false )
   {
      // the training input variables
      const char* inputVars[] = { "track_chi2", "abs(track_displX)", "abs(track_displY)", "track_nbHit", "abs(track_p)", "abs(track_tx)", "abs(track_ty)", "seed_chi2PerDoF", "seed_pt", "seed_nLHCbIDs", "seed_nbIT" };
