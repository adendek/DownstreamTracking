...
StatusCode PatMPLFinalSelectionTool::initialize()
{
  ...
  /*m_parametersName= { "track_chi2", "abs(track_displX)", "abs(track_displY)",
...
void PatMPLFinalSelectionTool::buildNetworkStructure()
{
...
inline double PatMPLFinalSelectionTool::GetMvaValue__( const std::vector<double>& inputValues ) const
{
...
double PatMPLFinalSelectionTool::ActivationFnc(double x) const {
   // rectified linear unit
   return x>0 ? x : 0;
}
double PatMPLFinalSelectionTool::OutputActivationFnc(double x) const {
   // sigmoid
   return 1.0/(1.0+exp(-x));
}

inline double PatMPLFinalSelectionTool::getMvaValue( const std::vector<double>& inputValues )
{
    double retval=0;
         if (IsNormalised()) {
            // normalise variables
...
void PatMPLFinalSelectionTool::InitTransform_1()
{
...
//_______________________________________________________________________
inline void PatMPLFinalSelectionTool::Transform_1( std::vector<double>& iv, int cls) const
{
...
//_______________________________________________________________________
void PatMPLFinalSelectionTool::InitTransform()
{
   InitTransform_1();
}

//_______________________________________________________________________
void PatMPLFinalSelectionTool::Transform( std::vector<double>& iv, int sigOrBgd ) const
{
