...
StatusCode PatLongLivedTracking::initialize() {
  ...
  m_finalSelectionMVATool= tool<IPatMvaClassifier>(m_mvaFinalSelectionToolName);
...
StatusCode PatLongLivedTracking::execute() {
  ...
  for ( LHCb::Track* tr : myInTracks ) {
    ...
    for( PatDownTrack& track : m_trackCandidates ){
      ...
      double mvaValue= getMPLValue(track, tr);
      track.setMvaValue(mvaValue);
      ...
      if( UNLIKELY(m_withDebugTool) ){
          auto trackParameters=getTrackParameters(track);
          m_debugTool->trackTuple(trackParameters ,track.hits(), tr);
      }
    }

    std::sort( m_trackCandidates.begin(), m_trackCandidates.end(),
                   [](const PatDownTrack& a, const PatDownTrack& b){ return a.mvaValue() > b.mvaValue();});

    for( PatDownTrack& track : m_trackCandidates ){
      if( track.ignore() ) continue;
      if(track.mvaValue()<m_trackCut) break; // reject all track candidates
      storeTrack( track, finalTracks, track.track() );// store the best candidate
      break;
    }
...
double PatLongLivedTracking::evaluateFisher(  const LHCb::Track* track ){
   ...
   const unsigned int nbIT = std::count_if( track->lhcbIDs().begin(), track->lhcbIDs().end(),
    ...
    std::vector<double> vals = { track->chi2PerDoF(), track->p(), static_cast<double>(nLayers), track->pt(), static_cast<double>(track->nLHCbIDs()), static_cast<double>(nbIT), std::abs(slopes.y()) };

    return m_seedSelectionMVATool->getMvaValue( vals );
}
//=============================================================================
...
std::map<std::string, double> PatLongLivedTracking::getTrackParameters(const PatDownTrack& track )
{
    std::map<std::string, double> trackParameters;
    trackParameters.insert(std::pair<std::string, double>("track_p", track.momentum()));
    trackParameters.insert(std::pair<std::string, double>("track_chi2", track.chi2()));
    trackParameters.insert(std::pair<std::string, double>("track_nbHit", track.hits().size()));
    trackParameters.insert(std::pair<std::string, double>("track_x", track.xAtZ(m_zTTa)));
    trackParameters.insert(std::pair<std::string, double>("track_y", track.yAtZ( m_zTTa )));
    trackParameters.insert(std::pair<std::string, double>("track_tx", track.slopeX()));
    trackParameters.insert(std::pair<std::string, double>("track_ty", track.slopeY()));
    trackParameters.insert(std::pair<std::string, double>("track_displY", track.displY()));
    trackParameters.insert(std::pair<std::string, double>("track_displX", track.displX()));
    trackParameters.insert(std::pair<std::string, double>("track_errXMag", track.errXMag()));
    trackParameters.insert(std::pair<std::string, double>("track_errYMag", track.errYMag()));
    trackParameters.insert(std::pair<std::string, double>("track_mvaValue", track.mvaValue()));
    trackParameters.insert(std::pair<std::string, double>("track_isIgnored", track.ignore()));
...
double PatLongLivedTracking::getMPLValue( const PatDownTrack& track, LHCb::Track* trackSeed )
{
   const unsigned int nbIT = std::count_if( trackSeed->lhcbIDs().begin(), trackSeed->lhcbIDs().end(),
    ...
    std::vector<double> trackParametersVector={track.chi2(), fabs(track.displX()), fabs(track.displY()), static_cast<double>(track.hits().size()), 
			fabs(track.momentum()), 	fabs(track.slopeX()), fabs(track.slopeY()),
                        trackSeed->chi2PerDoF(),  trackSeed->pt(), static_cast<double>(trackSeed->nLHCbIDs()), static_cast<double>(nbIT)};		
    
    return m_finalSelectionMVATool->getMvaValue(trackParametersVector);
}
